...
for vv=1:length(listVar);
    ...
    doCf=~isempty(find(isfinite(z(:))));
    subplot(2,length(listVar),vv+length(listVar)); if doCf; contourf(x,y,z,[0:0.05:0.5]); end; grid on;
...
for vv=1:length(listVar);
  ...
  for ii=1:9;
    ...
    tmp1=(nansum(n,2)>1e4)*ones(1,size(n,2)); z(tmp1==0)=NaN;
    doCf=~isempty(find(isfinite(z(:))));
    subplot(3,3,ii); if doCf; contourf(x,y,z,[0:0.05:0.5]); end; grid on;
