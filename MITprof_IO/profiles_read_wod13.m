function [varargout]=profile_read_wod13(dataset,nf,m,varargin);
...
if m==0;
    ...
    data_cur=NaN*zeros(10000,5); %array that is used to store one profile
    ...
    for ii=1:length(var_cur);
        tmp1=deblank(cell2mat(var_cur(ii)));
        jj=find(strcmp(dataset.var_in,tmp1));
        if ~isempty(jj); v_cur=[v_cur; [jj ii]]; end;
    end;
    ...
    while ~isempty(data_tmp);
        ...
        qc_loc=zeros(1,5); qc_loc(v_cur(:,1))=data_tmp(v_cur(:,2)+1)';
    ...
    for ii=1:length(dataset.var_in);
        jj=find(v_cur(:,1)==ii);
        if ~isempty(jj);
            eval(['profileCur.' dataset.var_out{ii} '=data_cur(1:count_cur-1,jj)'';']);
            %eval(['profileCur.' dataset.var_out{ii} '_ERR=0*data_cur(1:count_cur-1,jj)'';']);
        else;
            eval(['profileCur.' dataset.var_out{ii} '=NaN*data_cur(1:count_cur-1,1)'';']);
            %eval(['profileCur.' dataset.var_out{ii} '_ERR=NaN*data_cur(1:count_cur-1,1)'';']);
        end;
    end;
